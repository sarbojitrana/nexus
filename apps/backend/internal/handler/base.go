...
import (
	...
	"github.com/sarbojitrana/nexus/internal/validation"
)
...
// Handler creates a new base handler
...
// HandlerFunc represents a typed handler function that process a request and returns a response
...
func (h FileResponseHandler) Handle(c echo.Context, result interface{}) error {
	data := result.([]byte)
	c.Response().Header().Set("Content-Disposition", "attacment; filename="+h.filename)
...
func (h FileResponseHandler) AddAttributes(txn *newrelic.Transaction, result interface{}) {
	if txn != nil {
		// http.status_code already set by tracing middleware
...
// handleRequest is the unified handler function that eliminates code duplicaiton
func handleRequest[Req validation.Validatable](
	...
	if err := validation.BindAndValidate(c, req); err != nil {
		...
		logger.Error().
			Err(err).
			Dur("valiation_duration", validationDuration).
